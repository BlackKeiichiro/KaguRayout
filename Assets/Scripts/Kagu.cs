...
abstract public class Kagu : MonoBehaviour {
	...
	protected void OnMouseDown(){
		...
		save_position = transform.position - Camera.main.ScreenToWorldPoint(new Vector3(Input.mousePosition.x, Input.mousePosition.y, mouse_position.z));
	}
	protected void OnMouseDrag(){
		...
		if(this.transform.position.y < stage.transform.position.y){
			...
			localposition.y = stage.transform.position.y + sizey;
